...
export default defineConfig(() => {
  return {
    ...
    plugins: [
      ...
      react(),
      salesforce(),
      // Mock Salesforce platform endpoints so local dev doesn't 401
      {
        name: 'mock-salesforce-platform',
        configureServer(server) {
          server.middlewares.use((req, res, next) => {
            if (req.url?.includes('/ui-api/session/csrf')) {
              res.setHeader('Content-Type', 'application/json');
              res.end(JSON.stringify({ token: 'mock-csrf-token-dev' }));
              return;
            }
            next();
          });
